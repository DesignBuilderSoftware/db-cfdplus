...
Usage
    ...
    Options:
      ...
      - \par -allFaceSets
        Write all existing faceSets along with -allPatches, which results in a
        single file with all patches and faceSets. This also means that only
        the mesh will be exported, i.e. without any fields besides patchID

      - \par -skipEmptyPatches
        Do not write the patches that have not faces assigned.
      ...
      - \par -excludeFaceSets
        Specify the faceSets (wildcards) that are to be excluded, should be
        loaded from the dictionary file "system/foamToVTKBCFDKDict".
        For example:
        \verbatim
             excludeFaceSets ( facelet_1 facelet_2 "wallFaces.*" );
        \endverbatim
        The double quotes denote a regular expression.

        \par -faceSetNormals \n
        if a list of faceSet + normal pairs is to be loaded from
        "system/foamToVTKBCFDKDict" from the keyword of the same name.
        Example:
        \verbatim
            faceSetNormals
            (
      ...
      - \par -manuallyTaggedFaceSets
        Specify the faceSets that are to be writting with specific patchID
        and cellZoneID codes, which should be loaded from the dictionary
        file "system/foamToVTKBCFDKDict".
        For example:
        \verbatim
             manuallyTaggedFaceSets
             (
                 {
                     faceSetName  theName1;
                     patchID   10;
                     cellZoneID   1;
                 }
                 {
                     faceSetName  theName2;
                     patchID   10;
                     cellZoneID   2;
                 }
             );
      ...
      - \par -writeCellZoneID
        Write the cellZoneID field. -1 is for any cells outside of a cellZone.
        Values equal or larger to 0 refer to cellZones.

      - \par -reportCellZoneStats
        Reports the volume for each cellZone.
...
)
{
    ...
    forAll(patches, patchi)
    {
        ...
        else if(skipEmptyPatches && pp.empty())
        {
            Info<< "    discarding patch with no faces " << patchi
                << " " << pp.name() << endl;
        }
        else
        {
...
int main(int argc, char *argv[])
{
    ...
    (
        "allFaceSets",
        "must be combined with -allPatches, which will write all existing "
        "faceSets along with the patches, all into a single file"
    );
    argList::addBoolOption
    (
        "skipEmptyPatches",
        "so that patches that do not have faces assigned, are not exported"
    );
    argList::addBoolOption
    (
        "writeCellZoneID",
        "Write the cellZoneID field. -1 is for any cells outside of a "
        "cellZone. Values equal or larger to 0 refer to cellZones."
    );
    argList::addBoolOption
    (
        "reportCellZoneStats",
    ...
    (
        "excludeFaceSets",
        "if a list of faceSets to exclude is to be loaded from "
        "'system/foamToVTKBCFDKDict' from the keyword of the same name. "
        "Example: 'excludeFaceSets ( facelet \".*WallFaces\" );'"
    );
    argList::addBoolOption
    (
        "manuallyTaggedFaceSets",
        "write the faces for the listed faceSets from the file "
        "'system/foamToVTKBCFDKDict' from the keyword of the same name. "
    );
    argList::addBoolOption
    (
        ...
        "if a list of faceSet + normal pairs is to be loaded from "
        "'system/foamToVTKBCFDKDict' from the keyword of the same name. "
        "Example: 'faceSetNormals ( patchName1  (1 0 0) );'"
    );
    ...
    const bool allFaceSets = args.optionFound("allFaceSets");
    const bool skipEmptyPatches = args.optionFound("skipEmptyPatches");
    const bool writeCellZoneID = args.optionFound("writeCellZoneID");
    const bool reportCellZoneStats = args.optionFound("reportCellZoneStats");
    const bool manuallyTaggedFaceSets =
        args.optionFound("manuallyTaggedFaceSets");

    if(!allPatches && allFaceSets)
    {
        FatalErrorInFunction
            << "-allFaceSets must be used with -allPatches"
            << exit(FatalError);
    }
    ...
    PtrList<dictionary> manuallyTaggedFaceSetsList;
    List<wordRe> excludeFaceSets;
    ListHashTable<vector,word> faceSetNormals;

    if
    (
        args.optionFound("excludeFaceSets")
     || args.optionFound("faceSetNormals")
     || manuallyTaggedFaceSets
    )
    {
        if(!foamToVTKDictHeader.typeHeaderOk<IOdictionary>(true))
        {
            FatalErrorInFunction
                << "system/foamToVTKBCFDKDict is missing. It's needed for"
                << " the option -excludeFaceSets"
                << exit(FatalError);
        }

        IOdictionary foamToVTKDict(foamToVTKDictHeader);

        if(args.optionFound("excludeFaceSets"))
        {
            foamToVTKDict.lookup("excludeFaceSets") >> excludeFaceSets;
            Info<< "Not including faceSets " << excludeFaceSets << nl << endl;
        }

        if(args.optionFound("faceSetNormals"))
        {
            foamToVTKDict.lookup("faceSetNormals") >> faceSetNormals;
            Info<< "Correlation list between faceSets and target normals"
                << faceSetNormals
                << nl << endl;
        }

        if(manuallyTaggedFaceSets)
        {
            foamToVTKDict.lookup("manuallyTaggedFaceSets")
                >> manuallyTaggedFaceSetsList;

            Info<< "Will export only the listed faceSets from"
                << "manuallyTaggedFaceSets..."
                << endl;
        }
    }
    ...
    forAll(timeDirs, timeI)
    {
        ...
        if (reportCellZoneStats)
        {
            Info<< "    Reporting cellZone volumes:" << endl;
            ...
            forAll(czMesh, zi)
            {
                if(!czMesh[zi].empty())
                {
                    labelList cells = czMesh[zi];
                    scalar volume = 0.0;
                    forAll(cells, ci)
                    {
                        volume += mesh.V()[cells[ci]];
                    }
            ...
            Info<< "    End of cellZone volume report." << endl;
        }
        ...
        // If manuallyTaggedFaceSets: write only the VTK with the selected
        // faceSets from manuallyTaggedFaceSetsList.
        if(manuallyTaggedFaceSets)
        {
            mkDir(fvPath/"manuallyTaggedFaceSets");
            ...
            {
                patchFileName =
                    fvPath/"manuallyTaggedFaceSets"/cellSetName
                  + "_"
                  + timeDesc
                  + ".vtk";
            }
            ...
            {
                patchFileName =
                    fvPath/"manuallyTaggedFaceSets"/"manuallyTaggedFaceSets"
                  + "_"
                  + timeDesc
                  + ".vtk";
            }

            Info<< "    manuallyTaggedFaceSets : " << patchFileName << endl;

            //all faceSets are exported by default
            manualFaceSetsWriter writer
            (
                vMesh,
                binary,
                patchFileName,
            ...
            // Write cellZoneID field
            writer.writeCellZoneIDs();
        ...
        if (doWriteInternal)
        {
            ...
            // Write mesh
            internalWriterBCFDK writer(vMesh, binary, vtkFileName);

            // cellID + volFields::Internal + VolFields (+ cellZoneID)
            vtkWriteOps::writeCellDataHeader
            (
                writer.os(),
                vMesh.nFieldCells(),
                1+nVolFields + (writeCellZoneID ? 1 : 0)
            );
        ...
        if(allPatches && allFaceSets)
        {
            ...
            Info<< "    Combined patches+faceSets : " << patchFileName << endl;

            //all faceSets are exported by default
            patchNFaceSetsWriter writer
            (
                vMesh,
                binary,
                nearCellValue,
                patchFileName,
                getSelectedPatches(patches, excludePatches, skipEmptyPatches),
                excludeFaceSets,
                faceSetNormals
            );
