...
Changes, Authors and Copyright
    Based on OpenFOAM's surfaceFeatures.
    Modified and developed for blueCFD(R)-Kernel
    Copyright (C) 2022 FSD blueCAPE Lda  http://www.bluecape.com.pt
------------------------------------------------------------------------------
License
...
namespace Foam
{
    ...
    void extractFeaturesSplitByPatch
    (
    ...
    {
        Info<< "  Splitting surface by patches:" << endl;
        ...
        forAll(patches, patchi)
        {
          ...
          const boolList dummySurfBaffleRegions(1, false);

          // Extracting and writing a extendedFeatureEdgeMesh
          extendedFeatureEdgeMesh feMesh
          (
            extractFromSurface(subSurf, geometricTestOnly, includedAngle),
            runTime,
            prefixName + "." + patchName + ".extendedFeatureEdgeMesh",
            dummySurfBaffleRegions
          );

          Info<< "    Feature set info for patch: " << patchName << nl;
    ...
    )
    {
        ...
        const Switch extractByPatch =
            dict.lookupOrDefault<Switch>("extractByPatch", "off");
        ...
        if (extractByPatch)
        {
            ...
            extractFeaturesSplitByPatch
            (
              surfaceFileName.lessExt().name(),
              runTime,
              surf,
              geometricTestOnly,
              includedAngle
