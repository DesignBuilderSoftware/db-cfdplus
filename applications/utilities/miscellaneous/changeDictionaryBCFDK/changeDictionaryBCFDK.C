...
Usage
    ...
    Options:
      ...
      - \par -dontAddBoundaries
        Do not add new boundaries to the boundary file.
Note
...
int main(int argc, char *argv[])
{
    ...
    forAll(times, timei)
    {
        ...
        const bool dontAddBoundaries = args.optionFound("dontAddBoundaries");
        if (dontAddBoundaries)
        {
            Info<< "Not adding new boundaries to the file boundary."
                << endl;
        }
        ...
        forAllConstIter(dictionary, replaceDicts, fieldIter)
        {
            ...
            if (fieldName == "boundary")
            {
                ...
                // Add remaining entries (unless asked not to do it)
                if (!dontAddBoundaries)
                {
