...
Changes, Authors and Copyright
    Based on OpenFOAM's createBaffles.
...
Description
    Makes internal faces into boundary faces and orients the faceZone if
    necessary. Does not duplicate points, unlike mergeOrSplitBaffles.
...
int main(int argc, char *argv[])
{
    ...
    forAll(selectors, selectorI)
    {
        ...
        //orient faceZone if outsidePoint exists
        if (selectors[selectorI].dict().found("outsidePoint"))
        {
            const point outsidePoint(selectors[selectorI].dict()["outsidePoint"]);
            const faceZone& fZone = mesh.faceZones()[zoneID];
            #include "orientFaceZone.H"
        }
    }
