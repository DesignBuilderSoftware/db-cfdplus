...
Changes, Authors and Copyright
    Based on OpenFOAM's snappyHexMesh.
    Modified and developed for blueCFD(R)-Kernel
    Copyright (C) 2013-2018 FSD blueCAPE Lda  http://www.bluecape.com.pt

    Includes porting modifications made for blueCFD-Core:
        - 2014-02-21 FSD blueCAPE Lda: Modifications for blueCFD-Core 2.3
        - 2016-08-01 FSD blueCAPE Lda: Modifications for blueCFD-Core 2016-1
------------------------------------------------------------------------------
License
...
int main(int argc, char *argv[])
{
    ...
    // bCFDK's shared dictionary for final reporting of events
    IOdictionary sharedReportDict
    (
        IOobject
        (
    ...
    //Create folder for reporting issues
    if (Pstream::master())
    {
        mkDir
        (
    ...
    const Switch wantFaceSets(meshDict.lookup("addFaceSets"));
    ...
    if(wantFaceSets)
    {
        ...
        Info<< "Adding faceSets..." << endl;

        meshRefiner.compileAndCreateFaceSets();

        Info<< "faceSets added in = "
            << timer.cpuTimeIncrement() << " s." << endl;
    }
    ...
    //Write final report on master processor
    if (Pstream::master())
    {
        sharedReportDict.regIOobject::write();
    }
