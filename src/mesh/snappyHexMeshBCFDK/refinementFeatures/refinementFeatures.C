...
)
{
    forAll(featDicts, featI)
    {
        ...
        includeTwoEdgePoints_[featI] =
            dict.lookupOrDefault("includeTwoEdgePoints",false);
        ...
        else
        {
            ...
            // FSD blueCAPE 2014/18: enforce corner points on coplanar patches
            const label minEdgesOnPointCount =
                includeTwoEdgePoints_[featI] ? 1 : 2;

            labelList oldToNew(eMesh.points().size(), -1);
            DynamicField<point> newPoints(eMesh.points().size());
            forAll(pointEdges, pointi)
            {
                // FSD blueCAPE 2014/18: "2" changed to minEdgesOnPointCount
                if (pointEdges[pointi].size() > minEdgesOnPointCount)
                {
...
:
    ...
    includeTwoEdgePoints_(featDicts.size()),
