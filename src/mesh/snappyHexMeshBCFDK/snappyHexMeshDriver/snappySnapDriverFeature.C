...
namespace Foam
{
    ...
    // ---------------
    // FSD blueCAPE 5th of June 2014: Added this class for
    //- combineReduce operator for getting the minimum for listLists.
    class listListMin
    {
    public:
    ...
    forAll(pfSurfNormals, i)
    {
        ...
        if (magSqr(fDisp) < sqr(snapDist[pointi]) && mag(fSNormal) > vSmall)
        {
            ...
            else
            {
                ...
                //-------------------------------------------------------------
                // 2014/18 FSD blueCAPE: bug fix that avoids having 2 coplanar
                //points being used for determining a feature line or point.
                ...
                else if (surfacePoints.size() == 1)
                {
                    //Only add another point if it doesn't have an identical
                    //normal
                    if(fabs(fSNormal&surfaceNormals[0])<(1.0-0.001))
                    {
                        surfacePoints.append(pt);
                        faceToNormalBin[i] = surfaceNormals.size();
                        surfaceNormals.append(fSNormal);
...
void Foam::snappySnapDriver::patchCornerAdjustments
(
    const bool multiRegionFeatureSnap,
...
) const
{
    ...
    {
        labelList pointStatus(pp.nPoints(), -1);
        scalarListList attractionDistances(features.size());

        forAll(features, featI)
        {
            label nFeatPoints = features[featI].points().size();
            attractionDistances[featI].setSize(nFeatPoints, VGREAT);
        }
            ...
            if (featI != -1)
            {
                ...
                (
                  ...
                  ||
                    (
                        distSqr
                      < attractionDistances[featI][featPointI]
                    )
                )
                {
                    pointAttractor[featI][featPointI] = pointI;
                    pointConstraints[featI][featPointI].first() = 3;
                    pointConstraints[featI][featPointI].second() = Zero;
                    attractionDistances[featI][featPointI] = distSqr;
                }
            }
        }

        //if running in parallel, then let's sort out which points should this
        //processor truly move
        if(Pstream::parRun())
        {
          scalarListList globalAttractionDistances(attractionDistances);

          //get the global list
          combineReduce(globalAttractionDistances, listListMin());

          //now let's check which points this processor should move and which
          //shouldn't move
          forAll(globalAttractionDistances, featI)
          {
              forAll(globalAttractionDistances[featI], featPointI)
              {
                  if(
                      pointAttractor[featI][featPointI] != -1
                      &&
                      (
                          globalAttractionDistances[featI][featPointI]
                        < attractionDistances[featI][featPointI]
                      )
                    )
                  {
                      //looks like this point is not for this processor to
                      //attract to, so let's reset the request attraction
                      pointAttractor[featI][featPointI] = -1;
                      pointConstraints[featI][featPointI] = pointConstraint();
                      attractionDistances[featI][featPointI] = VGREAT;
                  }
              }
          }
...
) const
{
    ...
    // Patch corner readjustments for those in the same plane surface
    // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    // Mostly overrides the pointAttractor.
    // Hack put in by FSD blueCAPE Lda on June 2014, later revised
    // in April 2018.
    patchCornerAdjustments
    (
        multiRegionFeatureSnap,
...
void Foam::snappySnapDriver::writeUnsnappedToVTK
(
...
{
    //Need to gather all points and scalars...
    List<pointField> gatheredPoints(Pstream::nProcs());
    gatheredPoints[Pstream::myProcNo()] = pointField(pointList);
    Pstream::gatherList(gatheredPoints);

    List<scalarField> gatheredDists(Pstream::nProcs());
    gatheredDists[Pstream::myProcNo()] = scalarField(minNonSnappedDist);
    Pstream::gatherList(gatheredDists);
    ...
    if (Pstream::master())
    {
        ...
        scalarField allDists = ListListOps::combine<scalarField>
        (
            gatheredDists,
        ...
        coordSet unsnappedPointsSet(
            "unsnappedPoints",
            "xyz"
        );
        unsnappedPointsSet.transfer(allPoints);

        wordList fieldNames(1, "MinimumSetDistance");
        fieldNames.append("Iteration");

        const scalarField iteration(allDists.size(), vtkIter);

        List<const scalarField*> valuePtrs(2);
        valuePtrs[0] = &allDists;
        valuePtrs[1] = &iteration;

        autoPtr<writer<scalar>> scalarFormatterPtr_
        (
            writer<scalar>::New("vtk")
        );

        Info
            << "WARNING: Writing position list and distances to VTK file "
            << vtkFilePath
            <<", for points which could not be snapped onto a surface."
            << endl;
