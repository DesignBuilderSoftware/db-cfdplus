...
)
{
    ...
    if (returnReduce(localPoints.size(), sumOp<label>()) > 0)
    {
        ...
        // FSD blueCAPE Lda 2023: write the list of non-snapped points into
        // the assigned VTK file
        // Modified how we keep track of these points, since it no longer makes
        // sense to output to screen this info
        if(!vtkFilePath.empty())
        {
            DynamicList<vector> pointList;
            DynamicList<scalar> minNonSnappedDist;
            ...
            forAll(snapSurf, pointi)
            {
                if (snapSurf[pointi] == -1)
                {
                    ...
                    */
                    ...
                    pointList.append(localPoints[pointi]);
                    minNonSnappedDist.append(minSnapDist[pointi]);
                }
            }

            //Also keep a record of how many there are already
            label totalCount =
                returnReduce(pointList.size(), sumOp<label>());

            //Write it to VTK file
            //and keep track of number of points in bCFDK's
            //on-RAM shared dictionary
            dictionary& sharedReportDict =
                mesh.lookupObjectRef<dictionary>(
                    "snappyHexMeshBCFDKReportDict"
                );

            sharedReportDict.add
            (
                "LastIterNumFailedSnapPoints",
                totalCount,
                true
            );
            ...
            if (totalCount > 0)
            {
                ...
                if(sharedReportDict.found("TotalNumFailedSnapPoints"))
                {
                    totalCount += sharedReportDict.
                        lookup<label>("TotalNumFailedSnapPoints");
                }

                sharedReportDict.add
                (
                    "TotalNumFailedSnapPoints",
                    totalCount,
                    true
                );
            }
...
)
{
...

    {
        ...
        for (label iter = 0; iter < nFeatIter; iter++)
        {
            ...
            //Write only to master path
            const fileName vtkFilePrefix
            (
                mesh.time().constant() /
                "meshIssues" /
                "failedSnapPoints_"
            ...
            (
                ...
                vtkFilePrefix + name(iter) + ".vtk"
            );

            {
                // FSD blueCAPE Lda 2023: set flag for this issue into bCFDK's
                // on-RAM shared dictionary
                dictionary& sharedReportDict =
                    mesh.lookupObjectRef<dictionary>(
                        "snappyHexMeshBCFDKReportDict"
