...
) const
{
    ...
    if (totNCells >= maxGlobalCells)
    {
        ...
        // FSD blueCAPE Lda 2022: store the totNCells into the bCFDK's
        // on-RAM shared dictionary
        dictionary& sharedReportDict =
            mesh_.lookupObjectRef<dictionary>("snappyHexMeshBCFDKReportDict");
        sharedReportDict.add("maximumGlobalCellsReached", totNCells, true);
    }
    else
    {
