...
void Foam::meshRefinement::compileAndCreateFaceSets()
{
    // This assumes that the mesh is ready to be mapped to faceSets

    labelList namedRegionSurfaceIndex(mesh_.nFaces(), -1);
    labelList nSurfRegionFaces(surfaces_.nRegions(), 0);
    ...
    Info<< "Surfaces that will be associated to faceSets:" << endl;
    forAll(namedSurfaces, i)
    {
        label surfI = namedSurfaces[i];

        Info<< "  Surface : " << surfaces_.names()[surfI] << nl
            << "      faceZone : " << surfZones[surfI].faceZoneName() << endl;
    }
    ...
    // ~~~~~~~~~~~~~~~~~~~~~~~~~
    // This block of code is copy-paste-adapted from meshRefinement::zonify.

    {
        ...
        forAll(testFaces, i)
        {
            ...
            if (surface1[i] != -1)
            {
                ...
                {
                    namedRegionSurfaceIndex[faceI] = 
                        surfaces_.globalRegion(surface2[i], region2[i]);
                    nSurfRegionFaces[namedRegionSurfaceIndex[faceI]]++;
                }
                else
                {
                    namedRegionSurfaceIndex[faceI] = 
    ...
    //Sync namedRegionSurfaceIndex, just in case there is a surface closer on
    //another processor.
    syncTools::syncFaceList
    (
        mesh_,
        namedRegionSurfaceIndex,
        maxEqOp<label>()
    );

    // Finally create faceSets for each region
    createFaceSets(namedRegionSurfaceIndex, nSurfRegionFaces);
}
