...
void Foam::meshRefinement::createFaceSets
(
    const labelList &namedRegionSurfaceIndex,
    labelList &nSurfRegionFaces
) const
{
    //Now let's create the faceSets
    const List<wordList>& regionOnSurfaceNames =
        surfaces_.geometry().regionNames();
    wordList globalRegionNames;
    labelList globalRegionToSurfaceRegion;
    List<labelList> faceSetsFaceIdLists(nSurfRegionFaces.size());
    labelList maskForGlobalList; //this helps eliminate repeated names

    //create the global list of region names
    forAll(regionOnSurfaceNames, i)
    {
        globalRegionNames.append(regionOnSurfaceNames[i]);
    }

    //create backward reference list between region and origin surface
    globalRegionToSurfaceRegion.setSize(globalRegionNames.size(), -1);
    label globalIndex = 0;
    forAll(regionOnSurfaceNames, i)
    {
        label regionSize = regionOnSurfaceNames[i].size();
        for(label j=0; j<regionSize; j++)
        {
          globalRegionToSurfaceRegion[globalIndex] = i;
          globalIndex++;
        }
    }

    //create a reference list which helps merge information for repeated names
    maskForGlobalList.setSize(globalRegionNames.size(), -1);
    forAll(globalRegionNames, i)
    {
        word &currentName = globalRegionNames[i];

        //until proven otherwise
        maskForGlobalList[i] = i;

        //Have to "manually" search backwards for the first occurrence of this
        //name, if such exists
        for(label j=i-1; j>=0; j--)
        {
          if(currentName.match(globalRegionNames[j]))
          {
            maskForGlobalList[i] = j;
            Info<< "Repeated found: " << currentName
                << " = " << i << " vs " << j
                << endl;
          }
        }
    ...
    //Allocate the faceID lists per region
    forAll(nSurfRegionFaces, i)
    {
        if (debug)
        {
            Pout<< "Region as faceSet: "
                << globalRegionNames[i]
                << ", nFaces:" << nSurfRegionFaces[i] << nl;
        }

        //If the surface is unique or at least the first occurrence, then
        //kickstart it
        if(maskForGlobalList[i]==i)
        {
            faceSetsFaceIdLists[i].setSize(nSurfRegionFaces[i], -1);
            nSurfRegionFaces[i] = 0;
        }
        //If it's a duplicate surface, then extend the list for the primary
        //item. The one associated to the repeated surface will not be
        //populated.
        else
        {
            label primaryI = maskForGlobalList[i];
            faceSetsFaceIdLists[primaryI].resize(
                faceSetsFaceIdLists[primaryI].size() + nSurfRegionFaces[i], 
                -1
                );

            //We need to reset this one, since we will count this in the
            //primary entry
            nSurfRegionFaces[i] = 0;
        }
    }
    ...
    //populate faceSetsFaceIdLists
    forAll(namedRegionSurfaceIndex, i)
    {
        const label regionId = namedRegionSurfaceIndex[i];
        if(regionId >= 0)
        {
            label primaryI = maskForGlobalList[regionId];
            label appendPos = (nSurfRegionFaces[primaryI]++);
            faceSetsFaceIdLists[primaryI][appendPos] = i;
        }
    }

    //Create faceSets, but only for non-empty lists
    labelList totalFaceCounters(nSurfRegionFaces.size());
    forAll(nSurfRegionFaces, i)
    {
        totalFaceCounters[i] =
            returnReduce(nSurfRegionFaces[i], sumOp<label>());

        if(totalFaceCounters[i]>0)
        {
            Info<< "  Writing faceSet '" << globalRegionNames[i]
                << "', size "
                << totalFaceCounters[i]
                << endl;
        }

        if(nSurfRegionFaces[i]>0)
        {
            writeFaceSet(globalRegionNames[i], faceSetsFaceIdLists[i]);
        }
    }

    //Now we need to search, count and write faceSets that have faces assigned
    //to cellZones, regardless if they should or not be assigned to them
    const PtrList<surfaceZonesInfo>& surfZones = surfaces_.surfZones();
    const labelList& faceOwner = mesh_.faceOwner();
    const labelList& faceNeighbour = mesh_.faceNeighbour();
    const cellZoneMesh& cellZones = mesh_.cellZones();

    forAll(totalFaceCounters, i)
    {
        //check the total count, to ensure that all processors stay synced
        if(totalFaceCounters[i]>0)
        {
            forAll(surfZones, surfI)
            {
                const word& cellZoneName = surfZones[surfI].cellZoneName();
                if(cellZoneName.empty()) continue;

                word comboFaceSetName = 
                      globalRegionNames[i]
                    + "_ON_"
                    + cellZoneName;

                label zoneI = 
                    cellZones.findZoneID(cellZoneName);

                const cellZone& cZone = mesh_.cellZones()[zoneI];

                const labelList& faceIDs = faceSetsFaceIdLists[i];
                labelList faceIDTouchedList(faceIDs.size());
                label counter = 0;
                label counterBothSides = 0;

                forAll(faceIDs, j)
                {
                    //Warning: this assumes that there are no faceID values <= 0
                    label faceID = faceIDs[j];
                    label ownerCellID = -1;
                    label neighCellID = -1;
                    bool ownerCovered = false;
                    bool neighCovered = false;

                    if(faceID<faceOwner.size())
                    {
                      ownerCellID = faceOwner[faceID];
                      ownerCovered = findIndex(cZone, ownerCellID) >= 0;
                    }

                    if(faceID<faceNeighbour.size())
                    {
                      neighCellID = faceNeighbour[faceID];
                      neighCovered = findIndex(cZone, neighCellID) >= 0;
                    }

                    if(ownerCovered || neighCovered)
                    {
                        faceIDTouchedList[counter] = faceID;
                        counter++;
                    }
                ...
                //Get the total counters for sync
                label totalCounter = returnReduce(counter,sumOp<label>());
                label totalCounterBothSides = 
                    returnReduce(counterBothSides,sumOp<label>());

                //First we must report
                if(totalCounter>0)
                {
                    Info<< "  Contiguous faceSet '"
                        << globalRegionNames[i]
                        << "', with " << totalCounter
                        << " faces on cellZone '" << cellZoneName
                        << "', saved to faceSet '" << comboFaceSetName
                        << "'; zone covers both sides on "
                        << totalCounterBothSides
                        << " faces."
                        << endl;
                }

                //Now we can save, if applicable for this combination
                if(counter>0)
                {
                    faceIDTouchedList.setSize(counter);

                    writeFaceSet(comboFaceSetName, faceIDTouchedList);
                }
            }
        }
    }
...
void Foam::meshRefinement::writeFaceSet
(
    const word& name, 
    const labelList& faceIDList
) const
{
    if(debug)
    {
        //For debugging, report how many faces in a faceSet were
        //created per processor
        Pout<< "  Writing faceSet '" << name
            << "', size " << faceIDList.size() << endl;
    }
    ...
    currentSet().writeObject(
        IOstream::ASCII,
        mesh_.time().writeVersion(),
